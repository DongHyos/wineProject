...
import com.example.wine.jwt.JwtAccessDeniedHandler;
import com.example.wine.jwt.JwtAuthenticationEntryPoint;
import com.example.wine.jwt.JwtAuthenticationFilter;
...
public class WebSecurityConfig {
  ...
  private static final String[] PUBLIC_URLS = { //이 URL은 권한 검사안함
          "/main" , "/login", "/signup", "/", "/swagger-resources/**", "/swagger-ui.html","/swagger-ui/**",
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
    ...
    return http
            ...
            .and()
            .authorizeRequests().antMatchers("/main/assets/**", "/main/css/**", "/main/forms/**", "/main/img/**", "/main/vendor/**", "/main/js/**", "/main/scss/**").permitAll()// 다음 리퀘스트에 대한 사용권한 체크
...
//            .csrf().disable() // rest api이므로 csrf 보안이 필요없으므로 disable처리
  ...
  public WebSecurityCustomizer webSecurityCustomizer() { //시큐리티 filter 제외, 그러나 OncePerRequestFilter는 시큐리티 필터가 아니라서 로직실행
    return (web) -> web.ignoring().mvcMatchers(PUBLIC_URLS)
            .and()
            .ignoring().antMatchers("/main/assets/**", "/main/css/**", "/main/forms/**", "/main/img/**", "/main/vendor/**", "/main/js/**", "/main/scss/**");
  }
}
